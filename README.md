...
2. Publish resources (migrations and config files):
    ```shell
    php artisan webaune:publish:subscriptions
    ```

3. Execute migrations via the following command:
    ```shell
    php artisan webaune:migrate:subscriptions
    ```
...
**Laravel Subscriptions** has been specially made for Eloquent and simplicity has been taken very serious as in any other Laravel related aspect. To add Subscription functionality to your User model just use the `\Webaune\Subscriptions\Traits\HasPlanSubscriptions` trait like this:

```php
namespace App\Models;

use Webaune\Subscriptions\Traits\HasPlanSubscriptions;
...
```php
$plan = app('webaune.subscriptions.plan')->create([
    'name' => 'Pro',
...
```php
$plan = app('webaune.subscriptions.plan')->find(1);
...
$amountOfPictures = app('webaune.subscriptions.plan_feature')->where('slug', 'pictures_per_listing')->first()->value;

// Get feature value through the subscription instance
$amountOfPictures = app('webaune.subscriptions.plan_subscription')->find(1)->getFeatureValue('pictures_per_listing');
```
...
```php
$plan = app('webaune.subscriptions.plan')->find(2);
$subscription = app('webaune.subscriptions.plan_subscription')->find(1);
...
```php
// Find plan feature
$feature = app('webaune.subscriptions.plan_feature')->where('name', 'listing_duration_days')->first();
...
$subscriptions = app('webaune.subscriptions.plan_subscription')->byPlanId($plan_id)->get();

// Get bookings of the given user
$user = \App\Models\User::find(1);
$bookingsOfSubscriber = app('webaune.subscriptions.plan_subscription')->ofSubscriber($user)->get(); 

// Get subscriptions with trial ending in 3 days
$subscriptions = app('webaune.subscriptions.plan_subscription')->findEndingTrial(3)->get();

// Get subscriptions with ended trial
$subscriptions = app('webaune.subscriptions.plan_subscription')->findEndedTrial()->get();

// Get subscriptions with period ending in 3 days
$subscriptions = app('webaune.subscriptions.plan_subscription')->findEndingPeriod(3)->get();

// Get subscriptions with ended period
$subscriptions = app('webaune.subscriptions.plan_subscription')->findEndedPeriod()->get();
```

### Models

**Webaune Subscriptions** uses 4 models:
...
- [Help on Email](mailto:razeek@webaune.com)
...
(c) 2023-2024 Webaune Solutions, Some rights reserved.
